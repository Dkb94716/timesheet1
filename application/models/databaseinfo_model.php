...
//        $this->db->order_by('name_of_company');
    ...
    function getShareholderRedemptionData($client_id) {
        $this->db->select('*');
        $this->db->from("db_shareholder_redemption");
        $this->db->where('client_id', $client_id);
        $this->db->order_by('shareholder_name');
        $query = $this->db->get();
        $result = $query->result_array();
        return $result;
    }
    
    function getCapitalRedemptionData($client_id) {
        $this->db->select('*');
        $this->db->from("db_capital_redemption_data");
        $this->db->where('client_id', $client_id);       
        $query = $this->db->get();
        $result = $query->result_array();
        return $result;
