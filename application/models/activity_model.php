...
class Activity_model extends CI_Model {
    ...
    function listActivityForTimesheet($activity_id=NULL){
        $this->db->select('*');
        $this->db->from('activity');
        $this->db->order_by('activity_name', 'asc');
        if(!empty($activity_id))
    ...
    function getDefaultActivity(){
        ...
        $this->db->order_by('activity_name', 'asc');
        $this->db->where('project_id', 0);
        $this->db->where_in('activity_name', ['Analysis','Bug Fixing','Calling','Daily Scrum Meeting','Documentation','Followup','Meeting','Planning','Internal Review','Project Status Report','Project Review','QA','Review','Team Coordination','Lead generation']);
        $query = $this->db->get();
        $result = $query->result_array();
        if ($query->num_rows() == 0) {
            return false;
