...
class DocumentImpl implements BaseDocument {
    ...
    public function createDocument(Document $okmDocument, $is) {
        ...
        $documentXML->addChild('nodeClass', $okmDocument->getNodeClass());
        ...
        foreach ($okmDocument->getCategories() as $category) {
            ...
            $categoryXML->addChild('style', $category->getStyle());
        }
    ...
    public function getDocumentProperties($docId) {
        ...
        if ($position) {
            ...
            if ($error == 'PathNotFoundException') {
    ...
    public function setLanguage($docId, $lang) {
        $uri = UriHelper::getUri($this->host, UriHelper::DOCUMENT_SET_LANGUAGE);
        $uri .= '?docId=' . $docId . '&lang=' . $lang;
        $client = Request::put($uri);
        $this->getClient($client);
    }

    public function setTitle($docId, $title) {
        $uri = UriHelper::getUri($this->host, UriHelper::DOCUMENT_SET_TITLE);
    ...
    public function setDocumentNodeClass($docId, $ncId) {
        $uri = UriHelper::getUri($this->host, UriHelper::DOCUMENT_SET_NODECLASS);
        $uri .= '?docId=' . $docId . '&ncId=' . $ncId;
        $client = Request::put($uri);
        $this->getClient($client);
    }
    ...
    public function getDetectedLanguages() {
        $uri = UriHelper::getUri($this->host, UriHelper::DOCUMENT_GET_DETECTED_LANGUAGES);
        $client = Request::get($uri);
        $response = $this->getClient($client);
        $languages = array();
        foreach ($response->body as $languageXML) {
            $languages[] = (string) $languageXML;
        }
        return $languages;
    }
    ...
    public function phpDocument($documentXML) {
        ...
        $document->setNodeClass((int) $documentXML->nodeClass);
        $document->setNodeClass($documentXML->nodeClass);
    ...
    public function phpFolder($folderXML) {
        ...
        $folder->setNodeClass((int) $folderXML->nodeClass);
        ...
        $folder->setStyle((int) $folderXML->style);
        return $folder;
    }
