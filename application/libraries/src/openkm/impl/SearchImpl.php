...
class SearchImpl implements BaseSearch {
    ...
    public function saveSearch(QueryParams $params) {
        ...
        $queryParamsXML->addChild('folder', $params->getFolder());
        $queryParamsXML->addChild('folderRecursive', $params->isFolderRecursive());
        $queryParamsXML->addChild('queryName', $params->getQueryName());
        $queryParamsXML->addChild('user', $params->getUser());
        $queryParamsXML->addChild('notes', $params->getNotes());
    ...
    public function makeUri(QueryParams $queryParams, $uri) {
        ...
        if ($queryParams->getLanguage() != '') {
            $uri .= '&language=' . $queryParams->getLanguage();
        }
        if ($queryParams->getMimeType() != '') {
            $uri .= '&mimeType=' . $queryParams->getMimeType();
        }
        if ($queryParams->getTitle() != '') {
            $uri .= '&title=' . $queryParams->getTitle();
        }
    ...
    public function phpQueryResult($queryResultXML) {
        ...
        $queryResult->setScore($queryResultXML->score);
        return $queryResult;
    }
    ...
    public function phpQueryParams($queryParamsXML) {
        ...
        $queryParams->setFolder((string) $queryParamsXML->folder);
        $queryParams->setFolderRecursive((string) $queryParamsXML->folderRecursive);
        $queryParams->setQueryName((string) $queryParamsXML->queryName);
        $queryParams->setUser((string) $queryParamsXML->user);
        $queryParams->setNotes((string) $queryParamsXML->notes);
        $categories = array();
